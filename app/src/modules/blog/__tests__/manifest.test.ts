...
test("blog offers a public-nav entry to /blog (issue 053) — enabled contributes, disabled does not", () => {
  const registry = createModuleRegistry([blogModule]);
  expect(registry.get(BLOG_MODULE_ID)!.errors).toEqual([]);
  expect(blogModule.publicNav).toEqual([{ label: "Blog", href: "/blog", order: 10 }]);

  const enabled = getActiveCapabilities(registry, [BLOG_MODULE_ID]).publicNav;
  expect(enabled).toHaveLength(1);
  expect(enabled[0]).toMatchObject({ label: "Blog", href: "/blog", moduleId: BLOG_MODULE_ID });
