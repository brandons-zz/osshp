...
import { storeChallenge, consumeChallenge } from "./challenges";
...
export async function buildAuthenticationOptions(
  ...
  await storeChallenge(db, "authentication", options.challenge); // W1
  return options;
}
...
/**
 * Verify an authentication response. On success the credential counter is
 * persisted (W6: a non-incrementing counter from a synced passkey is tolerated —
 * we never hard-fail on counter stasis, we only persist the max seen). The caller
 * issues a rotated session after success (S3).
 */
export async function verifyAuthentication(
  db: Db,
  ctx: { response: AuthenticationResponseJSON },
): Promise<AuthenticationResult> {
  const challenge = await consumeChallenge(db, "authentication"); // single-use
  if (!challenge) {
