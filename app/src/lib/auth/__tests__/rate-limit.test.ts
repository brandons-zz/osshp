...
test("reproduction: with hops=1 (today's default) a simulated 2-hop tunnel chain collapses two different real clients to the same key", () => {
  withTrustedHops("1", () => {
...
test("fix: with hops=2 (tunnel mode) two different real clients behind the same cloudflared IP resolve to distinct keys", () => {
  withTrustedHops("2", () => {
...
test("with hops=2, an attacker prepending extra spoofed entries still cannot control the selected entry", () => {
  withTrustedHops("2", () => {
    const cloudflaredIp = "172.20.0.5";
...
test("loginLimiter isolates two distinct simulated tunnel-mode clients from each other (issue 070 fix, hops=2)", () => {
  withTrustedHops("2", () => {
    const cloudflaredIp = "172.20.0.5";
    ...
    expect(attempt("1.1.1.1").allowed).toBe(true);
    expect(attempt("1.1.1.1").allowed).toBe(false);
    ...
    expect(attempt("2.2.2.2").allowed).toBe(true);
    expect(attempt("2.2.2.2").allowed).toBe(false);
  });
});
