...
import { buildTar, pathFitsUstar, type TarEntry } from "../tar";
...
// pathFitsUstar (issue 072) — a caller assembling entries (the exporter) uses
// this to pre-filter an unrepresentable path into a per-item manifest error
// instead of letting buildHeader() throw and abort the whole archive.

test("pathFitsUstar is true for an ordinary posts/<slug>.md path", () => {
  expect(pathFitsUstar("posts/hello-world.md")).toBe(true);
});

test("pathFitsUstar is true right up to the 100-byte tail limit", () => {
  // "posts/" has exactly one "/"; the only split puts the whole "<slug>.md"
  // tail in the 100-byte name field, so the boundary is byteLength(tail)<=100.
  const slug = "a".repeat(97); // 97 + ".md" (3) === 100
  expect(pathFitsUstar(`posts/${slug}.md`)).toBe(true);
});

test("pathFitsUstar is false one byte past the 100-byte tail limit", () => {
  const slug = "a".repeat(98); // 98 + ".md" (3) === 101 > 100
  expect(pathFitsUstar(`posts/${slug}.md`)).toBe(false);
});

test("pathFitsUstar matches buildTar's own throw/no-throw behavior", () => {
  const good = "posts/hello-world.md";
  const bad = "media/" + "a".repeat(300) + "/800.jpg";
  expect(pathFitsUstar(good)).toBe(true);
  expect(() => buildTar([{ path: good, data: Buffer.from("x") }])).not.toThrow();
  expect(pathFitsUstar(bad)).toBe(false);
  expect(() => buildTar([{ path: bad, data: Buffer.from("x") }])).toThrow();
});
