...
/**
 * True iff `path` can be represented in a USTAR header (issue 072). Exposed so
 * a caller assembling entries (e.g. the exporter) can pre-filter an
 * unrepresentable path into a per-item error instead of letting buildTar()
 * throw and abort the whole archive — the same "degrade one bad item, don't
 * abort everything" shape the exporter already applies to missing media
 * objects (manifest.mediaErrors).
 */
export function pathFitsUstar(path: string): boolean {
