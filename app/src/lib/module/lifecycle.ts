...
import { routeNavBase } from "./nav-guard";
import type {
  AdminNavEntry,
...
export interface ActiveCapabilities {
  ...
  /** Public masthead-nav suggestions from enabled modules (issue 053). */
  publicNav: Array<PublicNavEntry & { moduleId: string }>;
...
export function getActiveCapabilities(
  ...
  for (const id of enabledIds) {
    ...
    for (const n of mod.manifest.publicNav ?? []) {
      out.publicNav.push({ ...n, moduleId: id });
    }
...
// ── Disabled-module nav bases (issue 053 defect — dead public links) ─────────

/**
 * The public-route "nav bases" of every registered module that is NOT currently
 * enabled — the set a saved `site.nav` href is checked against so the public
 * masthead never renders a link into a disabled module's 404'd route (and the
 * admin nav editor can flag it). A module absent from `enabledIds` — disabled OR
 * invalid — contributes its public bases, since either way its public routes do
 * not resolve. Enabled modules contribute nothing (their routes are live).
 */
export function getDisabledModuleNavBases(
  registry: ModuleRegistry,
  enabledIds: readonly string[],
): string[] {
  ...
  for (const mod of registry.list()) {
    if (enabled.has(mod.manifest.id)) continue; // enabled → routes live, skip
    for (const r of mod.routes) {
      if (r.access !== "public") continue; // admin routes are never in public nav
      const base = routeNavBase(r.path);
