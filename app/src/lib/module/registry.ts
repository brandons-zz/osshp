...
export function validateManifest(manifest: ModuleManifest): string[] {
  ...
  // Public-nav suggestions (issue 053) must point at one of THIS module's own
  // PUBLIC routes — a module cannot offer the masthead a link to a route it does
  // not actually serve publicly (mirrors the admin-nav constraint above).
  const publicRoutePaths = new Set(
    routes
      .filter((r) => resolveRouteAccess(r.access) === "public")
      .map((r) => r.path),
  );
  for (const entry of manifest.publicNav ?? []) {
    if (!publicRoutePaths.has(entry.href)) {
      errors.push(
        `Public-nav href "${entry.href}" does not match a public route of this module.`,
      );
    }
  }
