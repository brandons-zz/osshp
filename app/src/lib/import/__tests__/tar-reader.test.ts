...
test("readArchive rejects a highly-compressible over-cap archive with a clean bounded rejection (decompression-bomb defense, issue 026)", async () => {
  ...
  // well past the cap — the shape a real zip-bomb upload would take. Gzipping
  // something bigger than the cap proves the bound is enforced DURING inflate
  // (streamed, chunk by chunk, aborted mid-stream), not just against the
  // final size after the whole payload was already materialized.
  //
  // The cap is injected via readArchive's test-only `maxTotalBytes` seam and
  // scaled down from the production 1 GB. The guard's abort logic is
  // size-independent (it fires the moment cumulative inflated bytes cross
  // whatever cap it was given), so a small cap exercises the identical
  // production code path. The previous version of this test drove the real
  // 1 GB cap — ~1 GB of allocate/gzip/inflate per run — which made it
  // wall-clock-sensitive: ~1s in isolation but a timeout under full-suite
  // parallel load, even with a 30s override (issue 058). Per team BKM,
  // assert behavior, never wall-clock.
  const cap = 4 * 1024 * 1024; // 4 MB test cap; production default stays MAX_TOTAL_BYTES
  const bomb = Buffer.alloc(cap + 1024 * 1024, 0);
  ...
  expect(gz.length).toBeLessThan(cap / 100);

  const entries = await readArchive(gz, cap);
  expect(entries.length).toBe(1);
  expect(isTarReadError(entries[0])).toBe(true);
  expect(isTarReadError(entries[0]) && entries[0].error).toContain(
    `exceeds ${cap} total bytes after decompression`,
  );
});
