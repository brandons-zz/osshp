...
import type { ValidatedGalleryEntry, ValidationResult } from "./types";
...
/**
 * Normalize a raw gallery-cover / gallery-entry media reference into its bare
 * storage key (the archive-relative "media/" prefix, if present, stripped).
 * Anything that isn't a non-empty string is not a resolvable reference.
 */
function normalizeMediaKey(value: unknown): string | null {
  if (typeof value !== "string") return null;
  const trimmed = value.trim();
  if (trimmed === "") return null;
  return trimmed.startsWith("media/") ? trimmed.slice("media/".length) : trimmed;
}

/**
 * Normalize a raw `gallery` field (issue 050) into an ordered list of
 * {key, alt, caption} entries. Array order is preserved — it becomes the
 * gallery's `position` at write time. A malformed individual entry (not an
 * object, or no resolvable `src`) is dropped rather than failing the whole
 * post — the same per-item leniency as normalizeTags, and for the same
 * reason: one bad photo entry shouldn't sink an otherwise-good post. Actual
 * `mediaId` resolution (source key -> a live media row) happens later, in
 * importer.ts, once media ingestion has run — this module has no DB access.
 */
function normalizeGallery(value: unknown): ValidatedGalleryEntry[] {
  if (!Array.isArray(value)) return [];
  const out: ValidatedGalleryEntry[] = [];
  for (const entry of value) {
    if (!isRecord(entry)) continue;
    const key = normalizeMediaKey(entry.src);
    if (key === null) continue;
    const alt = typeof entry.alt === "string" ? entry.alt : "";
    const caption = typeof entry.caption === "string" ? entry.caption : "";
    out.push({ key, alt, caption });
  }
  return out;
}
...
export function classifyAndValidate(
  ...
  return {
    ...
    isGallery: coerceBoolean(fields.isGallery, false),
    galleryCoverKey: normalizeMediaKey(fields.galleryCover),
    gallery: normalizeGallery(fields.gallery),
    body,
  };
}
