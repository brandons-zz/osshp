...
import { rehypeFigureCaption } from "./figureCaption";

// Minimal, safe extension of the default (GitHub-style) sanitize schema for
// captioned images (issue 077): `figure`/`figcaption` carry no attributes of
// their own (structural wrappers only), and the credit link the caption
// plugin constructs needs `rel`/`target` on `<a>` in addition to the
// defaultSchema's existing `href` (already protocol-filtered to http/https).
...
// rehypeFigureCaption runs BEFORE rehype-sanitize: it only restructures nodes
// remark-rehype already produced from trusted markdown syntax, and the schema
// above still validates everything it constructs immediately afterward.
