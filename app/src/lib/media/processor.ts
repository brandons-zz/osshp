...
export async function processImage(
  ...
  // Probe the source once. EXIF Orientation 5–8 stores the pixels rotated 90°,
  // so the raw metadata width/height are the PRE-rotation (sensor) axes. We
  // auto-orient every variant below (.rotate()), which swaps those axes — so the
  // width we compare responsive targets against must be the UPRIGHT width, or the
  // no-upscaling skip logic would reason about the wrong dimension.
  const meta = await sharp(input).metadata();
  const orientationSwapsAxes = (meta.orientation ?? 1) >= 5; // 5,6,7,8 rotate 90°
  const srcWidth = (orientationSwapsAxes ? meta.height : meta.width) ?? 0;
  ...
  return Promise.all(
    targets.map(async (targetWidth): Promise<ImageVariant> => {
      // .rotate() with no args auto-orients from the EXIF Orientation tag BEFORE
      // the resize, so a phone photo shot sideways/upside-down is baked upright in
      // the pixels and the now-meaningless Orientation tag is reset to 1. This is
      // the fix for photos rendering rotated (issue 055): stripping EXIF without
      // first applying the orientation left the pixels un-rotated but tag-less, so
      // the viewer had no signal to correct them. Auto-orienting here is also what
      // keeps the privacy floor intact — the orientation tag is normalised away,
      // not preserved. (HEIC arrives already upright: heic-convert/libheif applies
      // the HEIF irot transform during decode, so .rotate() is a safe no-op there.)
