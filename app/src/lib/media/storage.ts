...
export interface MediaStorage {
  ...
  /** Remove an object. Idempotent — deleting an absent key must not throw. */
...
class GarageStorage implements MediaStorage {
  ...
  async delete(key: string): Promise<void> {
    await this.ensureBucket();
    // removeObject is idempotent on Garage/S3 — a missing key is a no-op, so a
    // usage-aware delete or a replace that prunes stale variants never throws.
