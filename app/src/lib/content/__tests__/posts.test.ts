...
// Issue 064 regression: editing a post to REDUCE or CLEAR its tags must
// reconcile the post_tags rows to exactly the submitted set — the removed
// association must be gone (add works, remove must too), and the post must
// drop out of the removed tag's listing (/tags/<slug>). Covered for BOTH a
// blog article and a photo post, since both save through updatePost.
test("updatePost reconciles tags on add, reduce, and clear-all (blog + photo, issue 064)", async () => {
  for (const type of ["article", "photo-post"] as const) {
    const slug = `recon-${type}`;
    const post = await createPost(db, {
      title: `Recon ${type}`,
      slug,
      body: "x",
    ...
    expect(
      (await getPostBySlug(db, slug))!.tags.map((t) => t.slug).sort(),
    ).toEqual(["alpha", "beta"]);
    // Post is listed under BOTH tags.
    expect(
      (await listPublishedPosts(db, { tagSlug: "alpha" })).some(
        (p) => p.slug === slug,
      ),
    ).toBe(true);
    expect(
      (await listPublishedPosts(db, { tagSlug: "beta" })).some(
        (p) => p.slug === slug,
      ),
    ).toBe(true);

    // REDUCE: remove Beta, keep Alpha — the removed association must be dropped.
    await updatePost(db, post.id, { tags: [{ name: "Alpha", slug: "alpha" }] });
    expect((await getPostBySlug(db, slug))!.tags.map((t) => t.slug)).toEqual([
      "alpha",
    ]);
    // Post has DISAPPEARED from the removed tag's listing.
    expect(
      (await listPublishedPosts(db, { tagSlug: "beta" })).some(
        (p) => p.slug === slug,
      ),
    ...
    // CLEAR-ALL: an empty submitted set removes every remaining association.
    await updatePost(db, post.id, { tags: [] });
    expect((await getPostBySlug(db, slug))!.tags).toEqual([]);
    expect(
      (await listPublishedPosts(db, { tagSlug: "alpha" })).some(
        (p) => p.slug === slug,
      ),
