...
export function getDb(): Db {
  ...
  const sql = rawSql();
  // Wrap a postgres.js query executor (pooled `sql` or a transaction-scoped one)
  // into a Db.query, applying the same jsonb parity fix in both.
  const queryVia =
    (exec: Sql) =>
    async <Row = Record<string, unknown>>(
      ...
      const result = (await exec.unsafe(
        text,
        params as never[],
  ...
  cachedDb = {
    query: queryVia(sql),
    // postgres.js `sql.begin` runs the callback on ONE reserved connection
    // inside BEGIN … COMMIT, and issues ROLLBACK automatically if it throws —
    // exactly the all-or-nothing semantics the Db.transaction seam promises.
    transaction: <T>(fn: (tx: Db) => Promise<T>): Promise<T> =>
      sql.begin((txSql) =>
        fn({ query: queryVia(txSql as unknown as Sql) }),
      ) as Promise<T>,
  };
  return cachedDb;
}
