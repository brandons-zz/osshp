...
// Pure, dependency-free matcher (no server/registry code) — client-safe.
...
export interface SettingsFormProps {
  ...
  logoSrc: string;
  logoAlt: string;
  /** issue 053 — public-nav entries from enabled modules (Blog, Photos), offered
   *  as one-click "Add" chips so a published photo at /photos is reachable. */
  moduleNavSuggestions?: NavItem[];
  /** issue 053 defect — nav bases of DISABLED modules; a saved nav row whose
   *  href targets one is flagged (the public site already hides it). */
  disabledModuleNavBases?: string[];
}
...
interface NavRowEditorProps {
  ...
  disabled?: boolean;
  /** issue 053 — module-offered public-nav entries, shown as one-click "Add"
   *  chips for any whose href is not already in the nav. */
  suggestions?: NavItem[];
  /** issue 053 defect — disabled-module nav bases; a row whose href targets one
   *  is flagged as pointing at a disabled module (hidden from the public site). */
  disabledModuleBases?: string[];
}

function NavRowEditor({ items, onChange, disabled, suggestions, disabledModuleBases }: NavRowEditorProps) {
  ...
  function addSuggestion(s: NavItem) {
    onChange([...items, { label: s.label, href: s.href }]);
  }

  // issue 053 — only suggest module sections not already in the nav (compared by
  // href, so a hand-added /photos hides the chip too).
  const presentHrefs = new Set(items.map((i) => i.href.trim()));
  const openSuggestions = (suggestions ?? []).filter(
    (s) => !presentHrefs.has(s.href),
  );
    ...
    <div className="row-editor">
      {items.map((row, i) => {
        ...
        const disabledModule =
          !!row.href.trim() &&
          hrefTargetsDisabledModule(row.href.trim(), disabledModuleBases ?? []);
        return (
            ...
            <div className="row-editor-fields">
              ...
              <div className="field">
                ...
                <input
                  ...
                  aria-describedby={
                    [
                      unsafe ? `nav-href-warn-${i}` : null,
                      disabledModule ? `nav-href-disabled-${i}` : null,
                    ]
                      .filter(Boolean)
                      .join(" ") || undefined
                  }
                ...
                {disabledModule ? (
                  <span className="field-warn" id={`nav-href-disabled-${i}`} role="status">
                    Points to a disabled module — hidden from the public site until you
                    re-enable that module (or remove this item).
                  </span>
                ) : null}
              </div>
            </div>
      ...
      {openSuggestions.length > 0 ? (
        <div className="nav-suggestions">
          <span className="nav-suggestions-label" id="nav-suggestions-label">
            Add a section:
          </span>
          <div
            className="nav-suggestions-chips"
            role="group"
            aria-labelledby="nav-suggestions-label"
          >
  ...
  return (
    <form className="stack" onSubmit={handleSave}>
      ...
      <section className="shell-card stack">
        ...
        <p className="muted">
          Add, remove, and reorder nav items and social links. Enabled modules
          (Blog, Photos) offer one-click "Add a section" chips below — nothing is
          added to your nav until you choose it. Pages with "Show in navigation"
          enabled appear automatically after these entries.
        </p>
        ...
        <div className="field">
          ...
          <NavRowEditor
            ...
            disabled={busy}
            suggestions={props.moduleNavSuggestions}
            disabledModuleBases={props.disabledModuleNavBases}
          />
        </div>
